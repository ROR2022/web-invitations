...
export async function POST(
  ...
  try {
    ...
    if (fetchError) {
      return NextResponse.json(
        { error: `Error al obtener el paquete: ${fetchError.message}` },
        { status: 400 }
      );
    }
    ...
    const newState = !packageData.is_active;
    ...
    if (updateError) {
      return NextResponse.json(
        { error: `Error al actualizar el paquete: ${updateError.message}` },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Paquete ${newState ? 'activado' : 'desactivado'} correctamente`,
      is_active: newState
    });
  } catch (error: any) {
    return NextResponse.json(
